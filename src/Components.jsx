...
- "unico": tarefas pontuais que some quando concluída (ex: agendar consulta, comprar medicamento)`;
  ...
  try{
    ...
    const text=data.(content&&content[0]&&content[0].text)||"[]";
...
async function carregarAnaliseGenetica(pacienteId){
  try{
    const{data}=await supabase.from("documentos")
      ...
      .maybeSingle();
    if(!data)return null;
    return{analise:data.conteudo_json,pdfNome:data.(conteudo_json&&conteudo_json.pdfNome)||data.titulo};
  }catch(e){
...
function ChatIA({membro,systemPrompt,apiKey,placeholder,sugestoes,inicialMsg,pdfB64,pacienteId}){
  ...
  const send=async(text)=>{
    ...
    try{
      ...
      const data=await res.json();
      const resposta=data.(content&&content[0]&&content[0].text)||"Erro ao processar.";
  ...
  const handleSubmit=async()=>{
    ...
    try{
      if(mode==="register"){
        ...
        if(error)throw error;
        if(data.user){
          onLogin({userId:data.user.id,email,name:data.user.(user_metadata&&user_metadata.name)||email.split("@")[0]});
        }
      }
    }catch(e){
  ...
  const verificarAvaliacoesPendentes=async()=>{
    ...
    if((consultasRealizadas&&consultasRealizadas.length)>0){
      ...
      if(semAvaliacao.length>0){
        ...
        setModalCsat({agendamento_id:c.id,medico_nome:c.(medicos&&medicos.nome)||"seu médico"});
...
function ModuloGeneticista({form,scores,apiKey,pacienteId,systemPrompt,laudoState,setLaudoState}){
  ...
  const handlePdf=async(file)=>{
    ...
    try{
      ...
      const rawText=data.(content&&content[0]&&content[0].text)||"{}";
  ...
  const handleDocUpload=async(files)=>{
    ...
    for(const file of Array.from(files)){
      ...
      try{
        ...
        const data=await res.json();
        const parsed=JSON.parse((data.(content&&content[0]&&content[0].text)||"{}").replace(/```json|```/g,"").trim());
        // Salvar no Supabase
...
function ModuloHome({form,scores,setModulo,pacienteId}){
  ...
  const scrollParaAgendamento=()=>{
    agendamentoRef.current&&agendamentoRef.current.scrollIntoView({behavior:"smooth",block:"start"});
  };
  ...
  const handleSelecionarPrograma=async(prog)=>{
    ...
    const medicoPersonal=(pacData&&pacData.medicos);
  ...
  const handleAgendar=async()=>{
    ...
    const medicoId=await supabase.from("medicos").select("id").eq("nome",medicoSelecionado.nome).single().then(({data})=>(data&&data.id));
    if(medicoId){
        ...
        {horarioSelecionado&&(
          <Card style={{padding:"20px",background:T.goldFaint,border:"1px solid "+T.goldBorder}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
              {[
                ["Programa",(programaSelecionado&&programaSelecionado.nome)],
                ["Médico",(medicoSelecionado&&medicoSelecionado.nome)],
