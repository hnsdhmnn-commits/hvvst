...
export const EQUIPE=[
  {id:"enfermeira",nome:"Florence",titulo:"Enfermeira Coordenadora",cor:T.teal,bg:T.tealBg,icon:"🩺",descricao:"Coordena seu plano integral, motivação e acompanhamento MEV."},
...
export const MODULOS=[
  ...
  {id:"ana",label:"Falar com Florence",icon:"🩺",membro:"enfermeira"},
...
async function gerarPlanoInicial(pacienteId,form,apiKey){
  ...
  const prompt=`Você é Florence, enfermeira coordenadora do HVV. Com base no perfil abaixo, gere um plano de cuidado inicial com tarefas concretas em JSON.

PERFIL:
- Condições: ${((form&&form.condicoes)||[]).filter(c=>c!=="Nenhuma").join(", ")||"nenhuma"}
- Medicamentos: ${((form&&form.meds)||[]).filter(m=>m!=="Nenhum").join(", ")||"nenhum"}
- Sono: ${(form&&form.sono)||7}h, qualidade ${(form&&form.qual_sono)||5}/10
- Estresse: ${(form&&form.estresse)||5}/10
- Exercícios: ${(form&&form.freq_treino)||0}x/semana
- Dieta: ${(form&&form.dieta)||"não informado"}
- Rede de apoio: ${(form&&form.qualidade_rede)||"-"}/10
- Vida social: ${(form&&form.satisfacao_social)||"-"}/10
...
async function carregarAnaliseGenetica(pacienteId){
  try{
    const{data}=await supabase.from("documentos")
      ...
      .maybeSingle();
    if(!data)return null;
    return{analise:data.conteudo_json,pdfNome:(data.conteudo_json&&data.conteudo_json.pdfNome)||data.titulo};
  }catch(e){
...
function ChatIA({membro,systemPrompt,apiKey,placeholder,sugestoes,inicialMsg,pdfB64,pacienteId}){
  ...
  const send=async(text)=>{
    ...
    try{
      ...
      const data=await res.json();
      const resposta=(data.content&&data.content[0]&&data.content[0].text)||"Erro ao processar.";
    ...
    }finally{setLoading(false);setTimeout(()=>bottomRef.current&&bottomRef.current.scrollIntoView({behavior:"smooth"}),100);inputRef.current&&inputRef.current.focus();}
  };
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      <div style={{flex:1,overflowY:"auto",padding:"24px 28px 8px"}}>
        {msgs.map((msg,i)=>{const isUser=msg.role==="user";return(<div key={i} style={{display:"flex",flexDirection:isUser?"row-reverse":"row",gap:12,marginBottom:20,alignItems:"flex-start"}}>{!isUser&&<div style={{width:36,height:36,borderRadius:"50%",background:(eq&&eq.bg)||T.goldFaint,border:"1.5px solid "+(eq&&eq.cor)||T.gold+"30",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0,marginTop:2}}>{(eq&&eq.icon)||"◈"}</div>}<div style={{maxWidth:"74%",padding:"14px 18px",background:isUser?T.goldFaint:T.surface,border:"1px solid "+isUser?T.goldBorder:T.border,borderRadius:isUser?"16px 16px 4px 16px":"4px 16px 16px 16px",fontSize:13,color:T.ink,lineHeight:1.8,whiteSpace:"pre-wrap",boxShadow:T.shadowCard}}>{msg.loading?<span style={{display:"inline-flex",gap:5}}>{[0,1,2].map(j=><span key={j} style={{width:6,height:6,borderRadius:"50%",background:(eq&&eq.cor)||T.gold,display:"inline-block",animation:`blink 1.2s ease ${j*0.2}s infinite`}}/>)}</span>:msg.content}</div></div>);})}
        <div ref={bottomRef}/>
      </div>
      {msgs.length<=1&&(sugestoes&&sugestoes.length)>0&&(
        <div style={{padding:"0 28px 14px",display:"flex",gap:8,flexWrap:"wrap",flexShrink:0}}>
          {sugestoes.map((s,i)=>(<button key={i} onClick={()=>send(s)} style={{padding:"8px 16px",background:T.surface,border:"1px solid "+T.border,borderRadius:20,fontSize:12,color:T.inkMid,cursor:"pointer",fontFamily:T.fB,transition:"all 0.18s",boxShadow:T.shadowCard}} onMouseOver={e=>{e.currentTarget.style.borderColor=(eq&&eq.cor)||T.gold;e.currentTarget.style.color=(eq&&eq.cor)||T.gold;}} onMouseOut={e=>{e.currentTarget.style.borderColor=T.border;e.currentTarget.style.color=T.inkMid;}}>{s}</button>))}
        </div>
      )}
  ...
  const handleSubmit=async()=>{
    ...
    try{
      if(mode==="register"){
        ...
        if(data.user){
          ...
          try{
            ...
            const{error:insErr}=await supabase.from("pacientes").insert({
              ...
              medico_id:(medico&&medico.id)||"c0618c54-a555-4781-8e91-075f8898e54a",
              nome:name,email,
              plano:"Essential",
            });
        ...
        if(error)throw error;
        if(data.user){
          onLogin({userId:data.user.id,email,name:(data.user.user_metadata&&data.user.user_metadata.name)||email.split("@")[0]});
        }
      }
    }catch(e){
...
export function AppPrincipal({user,form,apiKey,pacienteId,onLogout}){
  ...
  const verificarAvaliacoesPendentes=async()=>{
    ...
    if((consultasRealizadas&&consultasRealizadas.length)>0){
      ...
      if(semAvaliacao.length>0){
        ...
        setModalCsat({agendamento_id:c.id,medico_nome:(c.medicos&&c.medicos.nome)||"seu médico"});
  ...
  const nome=(form&&form.nome)||user.name||"Executivo";
...
INSTRUÇÃO CRÍTICA: Estes dados ACABARAM de ser salvos. Você TEM acesso ao check-in de hoje. NÃO diga que não consegue ver os dados. Abra a conversa comentando diretamente sobre estes números — especialmente os pontos mais críticos. Se estresse >= 7, comente. Se sono <= 4, comente. Se vínculos <= 4, comente. Se o paciente deixou observações, responda a elas diretamente.`:"\nCheck-in de hoje: ainda não realizado.";
    ...
    return `Você é Florence, enfermeira coordenadora da equipe de saúde do Hospital Virtual Verde (HVV) — benefício Stone. Você é o ponto central de contato e tem acesso a TODOS os dados do paciente.
...
- Condições: ${((form&&form.condicoes)||[]).filter(c=>c!=="Nenhuma").join(", ")||"nenhuma"}
- Medicamentos: ${((form&&form.meds)||[]).filter(m=>m!=="Nenhum").join(", ")||"nenhum"}
- Sono habitual: ${(form&&form.sono)||7}h, qualidade ${(form&&form.qual_sono)||5}/10
- Estresse habitual: ${(form&&form.estresse)||5}/10
- Treino: ${(form&&form.freq_treino)||0}x/semana
- Dieta: ${(form&&form.dieta)||"não informada"}
- Rede de apoio: ${(form&&form.qualidade_rede)||"-"}/10
- Vida social: ${(form&&form.satisfacao_social)||"-"}/10
  ...
  return(
    <div style={{display:"flex",height:"100vh",background:T.bg,fontFamily:T.fB,color:T.ink,overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        {modulo==="farmaceutico"&&<ModuloChat membro="farmaceutico" form={form} scores={scores} apiKey={apiKey} pacienteId={pacienteId} systemPrompt={buildPrompt("farmaceutico")} inicialMsg={`Olá! Sou Rafael. Medicamentos: ${((form&&form.meds)||[]).filter(m=>m!=="Nenhum").join(", ")||"nenhum"}. Em que posso ajudar?`} sugestoes={["Verificar interações","Como tomar minha medicação?","Posso tomar vitaminas junto?"]}/>}
...
// A Florence pode adicionar, atualizar e remover tarefas do plano de cuidado
function ChatIAComTools({systemPrompt,apiKey,placeholder,sugestoes,inicialMsg,pacienteId,onPlanChange}){
  ...
  const executarTool=async(toolInput)=>{
    ...
    console.log("[ANA TOOL] Input recebido:", JSON.stringify(toolInput));
    ...
    try{
      if(acao==="adicionar"){
        ...
        return error?"Erro ao adicionar tarefa: "+error.message:"Tarefa adicionada ao plano.";
      }
      ...
      if(acao==="atualizar"&&tarefa_id){
        ...
        if(!error&&onPlanChange)onPlanChange();
        return error?"Erro ao atualizar: "+error.message:"Tarefa atualizada.";
      }
  ...
  const send=async(text)=>{
    ...
    const enviar=async(history,tentativa=1)=>{
      try{
        ...
        // Resposta final
        const resposta=data.content.find(c=>c.type==="text") && data.content.find(c=>c.type==="text").text||"Erro ao processar.";
        setMsgs(prev=>[...prev.slice(0,-1),{role:"assistant",content:resposta}]);
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100%",overflow:"hidden"}}>
      <div style={{flex:1,overflowY:"auto",padding:"24px 28px 8px"}}>
        {msgs.map((msg,i)=>{
          ...
          return(<div key={i} style={{display:"flex",flexDirection:isUser?"row-reverse":"row",gap:12,marginBottom:20,alignItems:"flex-start"}}>
            {!isUser&&<div style={{width:36,height:36,borderRadius:"50%",background:(eq&&eq.bg)||T.goldFaint,border:"1.5px solid "+(eq&&eq.cor)||T.gold+"30",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0,marginTop:2}}>{(eq&&eq.icon)||"🩺"}</div>}
            <div style={{maxWidth:"74%",padding:"14px 18px",background:isUser?T.goldFaint:T.surface,border:"1px solid "+isUser?T.goldBorder:T.border,borderRadius:isUser?"16px 16px 4px 16px":"4px 16px 16px 16px",fontSize:13,color:T.ink,lineHeight:1.8,whiteSpace:"pre-wrap",boxShadow:T.shadowCard}}>
              {msg.loading?<span style={{display:"inline-flex",gap:5}}>{[0,1,2].map(j=><span key={j} style={{width:6,height:6,borderRadius:"50%",background:(eq&&eq.cor)||T.gold,display:"inline-block",animation:`blink 1.2s ease ${j*0.2}s infinite`}}/>)}</span>:msg.content}
            </div>
          </div>);
        })}
        <div ref={bottomRef}/>
      ...
      <div style={{padding:"6px 28px",background:T.surfaceMid,borderTop:"1px solid "+T.border,flexShrink:0}}>
        <div style={{fontSize:10,color:T.inkFaint,lineHeight:1.6}}>✦ A Florence pode adicionar tarefas ao seu plano — basta pedir. ⚠️ Valide decisões clínicas com seu médico.</div>
      </div>
      <div style={{borderTop:"1px solid "+T.border,padding:"14px 28px",display:"flex",gap:10,alignItems:"flex-end",flexShrink:0,background:T.bgWarm}}>
        <textarea ref={inputRef} rows={1} placeholder={apiKey?placeholder:"Configure a API Key..."} value={input} onChange={e=>{setInput(e.target.value);e.target.style.height="auto";e.target.style.height=Math.min(e.target.scrollHeight,120)+"px";}} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send(input);}}} disabled={loading||!apiKey} style={{flex:1,background:T.surface,border:"1.5px solid "+T.border,borderRadius:10,padding:"12px 16px",color:T.ink,fontFamily:T.fB,fontSize:13,outline:"none",lineHeight:1.6,minHeight:44,maxHeight:120,overflow:"hidden",resize:"none",opacity:apiKey?1:0.5,boxShadow:T.shadowCard}}/>
        <button onClick={()=>send(input)} disabled={loading||!input.trim()||!apiKey} style={{width:44,height:44,borderRadius:10,background:(!loading&&input.trim()&&apiKey)?(eq&&eq.cor)||T.gold:"transparent",border:"1.5px solid "+(!loading&&input.trim()&&apiKey)?(eq&&eq.cor)||T.gold:T.border,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,color:(!loading&&input.trim()&&apiKey)?"#FFF":T.inkFaint,transition:"all 0.2s",flexShrink:0,fontWeight:700}}>{loading?"…":"↑"}</button>
...
function ModuloAna({form,scores,apiKey,checkinHoje,onCheckinSalvo,onPlanUpdate,pacienteId,getBuildPrompt,onPlanChange}){
  ...
  const nome=(form&&form.nome)||"Executivo";
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{borderBottom:"1px solid "+T.border,padding:"16px 28px",display:"flex",alignItems:"center",gap:14,background:T.surface,flexShrink:0,boxShadow:T.shadowCard}}>
        <div style={{width:42,height:42,borderRadius:"50%",background:eq.bg,border:"1.5px solid "+eq.cor+"40",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>{eq.icon}</div>
        <div><div style={{fontFamily:T.fD,fontSize:20,color:T.ink}}>Florence · Enfermeira Coordenadora</div><div style={{fontSize:9,color:T.green,letterSpacing:"0.15em"}}>● ONLINE · COORDENADORA GERAL · HISTÓRICO COMPLETO SALVO</div></div>
          ...
          <div style={{maxWidth:580,margin:"0 auto"}}>
            {checkinHoje?(
              <Card style={{padding:"28px",textAlign:"center",background:T.greenBg,border:"1px solid "+T.green+"30"}}>
                ...
                <Btn onClick={()=>setAba("chat")} variant="teal">CONVERSAR COM FLORENCE →</Btn>
              </Card>
            ):(
              <div>
                <div style={{fontFamily:T.fD,fontSize:24,color:T.ink,marginBottom:6}}>Check-in diário com Florence</div>
                ...
                <Card style={{padding:"28px",display:"flex",flexDirection:"column",gap:22}}>
                  ...
                  <TxtInput label="Observações para Florence (opcional)" placeholder="Algo que queira compartilhar..." value={ci.notas} onChange={v=>setCi(p=>({...p,notas:v}))}/>
...
${(analise&&analise.resumo)||"Tenho todos os achados em mãos."}
...
O que gostaria de saber?`;
  ...
  const send=async(text)=>{
    ...
    setMsgs(prev=>[...prev,userMsg,{role:"assistant",content:"",loading:true}]);
    setInput("");setLoading(true);
    const analiseResumida={resumo:(analise&&analise.resumo),nivel_risco_geral:(analise&&analise.nivel_risco_geral),achados:((analise&&analise.achados)||[]).slice(0,6),nutricao:(analise&&analise.nutricao),atividade:(analise&&analise.atividade),sono:(analise&&analise.sono),medicamentos:(analise&&analise.medicamentos),rastreamento:(analise&&analise.rastreamento)};
...
function ModuloGeneticista({form,scores,apiKey,pacienteId,systemPrompt,laudoState,setLaudoState}){
  const eq=EQUIPE.find(e=>e.id==="geneticista");
  const pdfB64=(laudoState&&laudoState.pdfB64)||null;
  const pdfNome=(laudoState&&laudoState.pdfNome)||null;
  const analise=(laudoState&&laudoState.analise)||null;
  ...
  const handlePdf=async(file)=>{
    ...
    try{
      ...
      console.log("analise resumo:",(parsed&&parsed.resumo&&parsed.resumo.slice(0,50)));
      if(pacienteId){
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{borderBottom:"1px solid "+T.border,padding:"16px 28px",display:"flex",alignItems:"center",gap:14,background:T.surface,flexShrink:0}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:10,alignItems:"center"}}>
          ...
          <Btn onClick={()=>fileRef.current&&fileRef.current.click()} variant="outline" style={{borderColor:T.purple,color:T.purple}}>{pdfB64?"TROCAR LAUDO":"CARREGAR LAUDO →"}</Btn>
      ...
      {aba==="chat"&&(!pdfB64?<div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,padding:40,textAlign:"center"}}><div style={{fontSize:48}}>🧬</div><div style={{fontFamily:T.fD,fontSize:22,color:T.inkMid}}>Carregue seu laudo primeiro</div><Btn onClick={()=>fileRef.current&&fileRef.current.click()} variant="outline" style={{borderColor:T.purple,color:T.purple}}>SELECIONAR PDF →</Btn></div>:<ChatGeneticista apiKey={apiKey} analise={analise} pdfNome={pdfNome} systemPrompt={systemPrompt}/>
  )}
    </div>
  );
}
...
function ModuloMensagens({pacienteId,nome}){
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          ...
          return(
            <div key={i} style={{display:"flex",flexDirection:isUser?"row-reverse":"row",gap:12,marginBottom:18,alignItems:"flex-start"}}>
              ...
              <div style={{maxWidth:"74%"}}>
                <div style={{fontSize:9,color:T.inkFaint,marginBottom:4,fontFamily:T.fB}}>{isAna?"FLORENCE · ENFERMEIRA":isMedico?"DR. DOHMANN":"VOCÊ"} · {new Date(msg.created_at).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit",hour:"2-digit",minute:"2-digit"})}</div>
      ...
      <div style={{borderTop:"1px solid "+T.border,padding:"14px 28px",display:"flex",gap:10,alignItems:"flex-end",flexShrink:0,background:T.bgWarm}}>
        <textarea ref={inputRef} rows={1} value={input} onChange={e=>{setInput(e.target.value);e.target.style.height="auto";e.target.style.height=Math.min(e.target.scrollHeight,120)+"px";}} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();enviar();}}} placeholder={`Escreva uma mensagem para ${nomeMedico||"seu médico"}...`} disabled={enviando} style={{flex:1,background:T.surface,border:"1.5px solid "+T.border,borderRadius:10,padding:"12px 16px",color:T.ink,fontFamily:T.fB,fontSize:13,outline:"none",lineHeight:1.6,minHeight:44,maxHeight:120,overflow:"hidden",resize:"none",boxShadow:T.shadowCard}}/>
...
function ModuloHome({form,scores,setModulo,pacienteId}){
  const HORARIOS=[
    {data:"Hoje",dataISO:new Date().toISOString().slice(0,10),slots:["09h00","10h30","14h00","15h30"].filter(s=>{var h=parseInt(s.slice(0,2)),m=parseInt(s.slice(3,5)),agora=new Date(),agoraMin=agora.getHours()*60+agora.getMinutes(),slotMin=h*60+m;return slotMin>agoraMin+30;})},
  ...
  useEffect(()=>{
    if(!pacienteId)return;
    supabase.from("pacientes").select("empresa_id").eq("id",pacienteId).single().then(({data})=>{
      if((data&&data.empresa_id))carregarProgramas(data.empresa_id).then(setProgramas);
    });
  },[pacienteId]);

  const scrollParaAgendamento=()=>{
    if(agendamentoRef.current)agendamentoRef.current.scrollIntoView({behavior:"smooth",block:"start"});
  };
  ...
  const handleSelecionarEspecialidade=(esp)=>{
    setModalEncaminhamento({nome:esp});
  };

  const handleSelecionarMedico=(m)=>{
    setMedicoSelecionado(m);
    setTela("horarios");
  };

  const handleAgendar=async()=>{
    if(!horarioSelecionado||!medicoSelecionado||!programaSelecionado||!pacienteId)return;
    setAgendando(true);
    const medicoId=medicoSelecionado.id&&medicoSelecionado.id.toString().startsWith("demo-")?null:medicoSelecionado.id;
    if(medicoId){
        ...
        <div style={{maxWidth:780,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:12,marginBottom:28}}>
            <div onClick={scrollParaAgendamento}
              ...
              <div style={{fontSize:12,color:T.inkMid,lineHeight:1.5,flex:1}}>Consulta com seu médico pessoal, programa de cuidado contínuo ou especialista parceiro.</div>
          ...
          <div ref={agendamentoRef}>
            ...
            <div style={{marginBottom:28}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                {programas.map(p=>{
                  const DESC={"saude-da-mulher":"Ginecologia, gestação, climatério","saude-do-homem":"Urologia, prevenção, cuidado geral","saude-da-crianca":"Pediatria, desenvolvimento, rotina","saude-emocional":"Psicologia, psiquiatria, sono, estresse","bem-estar":"Nutrição, atividade física, prevenção","emagrecimento":"Médico, nutri, atividade, comportamento","cirurgias":"Avaliação, 2ª opinião, preparo, pós-op"};
                  const PROF={"saude-da-mulher":"8 médicas","saude-do-homem":"6 médicos","saude-da-crianca":"5 médicos","saude-emocional":"12 profissionais","bem-estar":"9 profissionais","emagrecimento":"7 profissionais","cirurgias":"11 cirurgiões"};
                  return (
                    <div key={p.id} onClick={()=>handleSelecionarPrograma(p)}
                      ...
                      <div style={{fontSize:11,color:T.inkLight,lineHeight:1.5,marginBottom:8}}>{DESC[p.slug]||(p.descricao?p.descricao.slice(0,40):"")}</div>
            ...
            <div>
              ...
              <div onClick={()=>setModulo("plano")} style={{padding:"14px 16px",background:T.surface,border:"0.5px solid "+T.border,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10,cursor:"pointer"}}>
                <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <div onClick={()=>setModulo("documentos")} style={{padding:"14px 16px",background:T.surface,border:"0.5px solid "+T.border,borderRadius:10,cursor:"pointer"}}>
          ...
          {modalEncaminhamento&&(
            <div onClick={()=>setModalEncaminhamento(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:24}}>
              <div onClick={e=>e.stopPropagation()} style={{background:T.surface,borderRadius:16,padding:"28px 24px",maxWidth:400,width:"100%",boxShadow:"0 12px 40px rgba(0,0,0,0.15)"}}>
                <div style={{fontSize:20,marginBottom:8}}>🏥</div>
                <div style={{fontSize:16,fontWeight:600,color:T.ink,marginBottom:8}}>{modalEncaminhamento.nome}</div>
                <div style={{fontSize:13,color:T.inkMid,lineHeight:1.7,marginBottom:20}}>Para consultas com especialista em <strong>{modalEncaminhamento.nome}</strong>, você será encaminhado para um de nossos prestadores parceiros credenciados.</div>
                <div style={{padding:"12px 14px",background:T.greenBg,borderRadius:10,fontSize:12,color:T.green,marginBottom:20,lineHeight:1.6}}>✦ Seu médico pessoal será informado sobre este encaminhamento e acompanhará o resultado da sua consulta.</div>
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  <Btn variant="primary" style={{width:"100%",justifyContent:"center"}} onClick={()=>{setModalEncaminhamento(null);alert("Em breve você receberá o contato do prestador parceiro para agendamento. Seu médico pessoal foi notificado.");}}>Solicitar encaminhamento →</Btn>
                  <Btn variant="outline" style={{width:"100%",justifyContent:"center"}} onClick={()=>setModalEncaminhamento(null)}>Cancelar</Btn>
                </div>
              </div>
            </div>
          )}
        ...
        <div style={{maxWidth:700,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
            ...
            <div>
              ...
              <div style={{fontSize:13,color:T.inkMid}}>{(programaSelecionado&&programaSelecionado.nome)} · Este será seu médico pessoal neste programa</div>
            </div>
          </div>
          <div style={{padding:"10px 14px",background:T.goldFaint,border:"1px solid "+T.goldBorder,borderRadius:8,marginBottom:24,fontSize:12,color:T.inkMid}}>💡 Seu médico pessoal acompanha sua saúde ao longo do tempo — não é uma consulta avulsa. Escolha alguém com quem você queira construir um vínculo.</div>
        ...
        <div style={{maxWidth:700,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
            ...
            <div>
              ...
              <div style={{fontSize:13,color:T.inkMid}}>{(programaSelecionado&&programaSelecionado.nome)}</div>
            </div>
          </div>
          <Card style={{padding:"16px 18px",marginBottom:20,display:"flex",alignItems:"center",gap:14}}>
            ...
            <div style={{flex:1}}>
              <div style={{fontSize:14,color:T.ink,fontWeight:600}}>{(medicoSelecionado&&medicoSelecionado.nome)}</div>
              <div style={{fontSize:12,color:T.inkMid}}>{(medicoSelecionado&&medicoSelecionado.especialidade)||"Clínica Geral"} · CRM {(medicoSelecionado&&medicoSelecionado.crm)}</div>
            </div>
              ...
              <Card key={di} style={{padding:"16px 18px"}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  {dia.slots.map((slot,si)=>{
                    const key=di+"-"+si;
                    const sel=(horarioSelecionado&&horarioSelecionado.key)===key;
                    return (
                      <button key={si} onClick={()=>setHorarioSelecionado({key:key,hora:slot,data:dia.data,dataISO:dia.dataISO})}
          ...
          {horarioSelecionado&&(
            <Card style={{padding:"20px",background:T.goldFaint,border:"1px solid "+T.goldBorder}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
                {[["Programa",(programaSelecionado&&programaSelecionado.nome)],["Médico",(medicoSelecionado&&medicoSelecionado.nome)],["Data",horarioSelecionado.data],["Horário",horarioSelecionado.hora],["Tipo","Teleconsulta"],["Duração","30–40 minutos"]].map(([label,val])=>(
      ...
      <div style={{flex:1,overflowY:"auto",padding:"32px"}}>
        <div style={{maxWidth:700,margin:"0 auto"}}>
          <div style={{textAlign:"center",padding:"28px 0 24px"}}>
            ...
            <div style={{fontSize:14,color:T.inkMid}}>{(horarioSelecionado&&horarioSelecionado.data)} às {(horarioSelecionado&&horarioSelecionado.hora)} · {(programaSelecionado&&programaSelecionado.nome)}</div>
          </div>
          ...
          <Card style={{padding:"16px 18px",marginBottom:16,display:"flex",alignItems:"center",gap:12,background:T.tealBg,border:"1px solid "+T.teal+"30"}}>
            ...
            <div>
              <div style={{fontSize:13,color:T.ink,fontWeight:500,marginBottom:2}}>Florence foi notificada</div>
              <div style={{fontSize:12,color:T.inkMid,lineHeight:1.6}}>A Florence preparará um resumo do seu perfil de saúde e histórico de check-ins para o médico antes da consulta.</div>
            </div>
          </Card>
          <Card style={{padding:"18px 20px",marginBottom:16}}>
            <div style={{fontFamily:T.fD,fontSize:16,color:T.ink,marginBottom:14}}>Próximos passos</div>
            {[{n:1,text:"Verifique seu e-mail — o link da videochamada será enviado 24h antes"},{n:2,text:"Prepare seus exames e documentos recentes para mostrar ao médico"},{n:3,text:"Faça seu check-in no dia da consulta — o médico verá seus dados atualizados"},{n:4,text:"Entre no link 5 minutos antes do horário agendado"}].map(({n,text})=>(
    ...
    <div style={{flex:1,overflowY:"auto",padding:"32px"}}>
      <div style={{maxWidth:980,margin:"0 auto",display:"flex",flexDirection:"column",gap:20}}>
        ...
        {!checkinHoje?(
          ...
          <Card style={{padding:"16px 20px",background:T.greenBg,border:"1px solid "+T.green+"30"}}>
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:12}}>
              ...
              <Btn onClick={()=>setModulo("ana")} variant="outline" style={{fontSize:11}}>Conversar com Florence →</Btn>
            </div>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
              {inscricoes.map(insc=>{
                ...
                return(
                  <Card key={insc.id} style={{padding:"18px",borderLeft:"3px solid "+(prog&&prog.cor)||T.green,cursor:"pointer"}} onClick={()=>setModulo("ana")}>
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                      <div style={{width:36,height:36,borderRadius:10,background:(prog&&prog.cor)||T.green+"15",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>{(prog&&prog.icone)||"🩺"}</div>
                      <div>
                        <div style={{fontFamily:T.fD,fontSize:14,color:T.ink}}>{(prog&&prog.nome)}</div>
                    ...
                    <div style={{fontSize:11,color:T.inkMid,lineHeight:1.6}}>{(prog&&prog.descricao&&prog.descricao.slice)(0,80)}...</div>
                  </Card>
                );
              })}
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
              {programas.filter(p=>!isInscrito(p.id)).map(p=>(
                <Card key={p.id} style={{padding:"18px",borderLeft:"3px solid "+p.cor||T.green}}>
                  ...
                  <div style={{fontSize:11,color:T.inkMid,lineHeight:1.6,marginBottom:12}}>{(p.descricao?p.descricao.slice(0,90):"")}...</div>
        ...
        <Card style={{padding:"20px 24px",background:T.tealBg,border:"1px solid "+T.teal+"30",display:"flex",alignItems:"center",gap:16}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontFamily:T.fD,fontSize:18,color:T.ink,marginBottom:3}}>Florence · Enfermeira Coordenadora</div>
          ...
          <Btn onClick={()=>setModulo("ana")} variant="teal">FALAR COM FLORENCE →</Btn>
        </Card>
      ...
      <div style={{maxWidth:1000,margin:"0 auto"}}>
        ...
        {tarefas.length===0&&vacinasAplicaveis.length===0&&desfechos.length===0?(
          <Card style={{padding:"40px",textAlign:"center"}}>
            ...
            <div style={{fontSize:13,color:T.inkMid,lineHeight:1.9,maxWidth:420,margin:"0 auto 24px"}}>A Florence vai criar um plano personalizado com base no seu perfil de saúde. Leva menos de 30 segundos.</div>
...
function ModuloIntegracoes(){
  ...
  const allDone=(id)=>{const it=INTEGRACOES.find(i=>i.id===id);return (it&&it.passos).every((_,idx)=>isSD(id,idx));};
  return(
    <div style={{flex:1,display:"flex",overflow:"hidden"}}>
